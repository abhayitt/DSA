...
void commonDetail(CityData cityData, const char* corperation1, const char* corperation2) {
    ...
    for (int i = 0; i <SUBURBS; i++) {
        for (int j = 0; j <TOWNS; j++) {
            if ((strcmp(cityData.corporations[i][j][0], corperation1) == 0 && strcmp(cityData.corporations[i][j][1], corperation2) == 0) ||
                (strcmp(cityData.corporations[i][j][0], corperation2) == 0 && strcmp(cityData.corporations[i][j][1], corperation1) == 0)) {
    ...
    //share a suburb
    for (int i = 0; i <SUBURBS; i++) {
        if ((strcmp(cityData.corporations[i][0][0], corperation1) == 0 && strcmp(cityData.corporations[i][0][1], corperation2) == 0) ||
            (strcmp(cityData.corporations[i][0][0], corperation2) == 0 && strcmp(cityData.corporations[i][0][1], corperation1) == 0)) {
            strcpy(suburb_common, cityData.suburbs[i]);
            break;
        }
    }

    //Printing the common details:
    if (strlen(town_common) > 0) {
